...
/**
 * Classe que guarda os atributos e métodos de um label com animação
 * customizável.
 *
...
public class LabelAnimado extends JLabel implements Interagivel {
    ...
    private void escolheAnimacao(Animacao tipo) {
        switch (tipo) {
            ...
            case FADE -> {
                ...
                timerAnimacao = new Timer(5, e -> animacaoFade());
            }
            default -> {
                timerAnimacao = null;
                ajuste = null;
    ...
    @Override
    public void mudaFoco(boolean tipoFoco) {
        if(tipoFoco)
            startAnimacao();
        else
            stopAnimacao();
    ...
    public void paintComponent(Graphics g) {
        ...
        int gap = (int) Math.ceil(font.getSize() / 50);

        Color highlight = new Color(128, 128, 128, alpha);
        
        g2d.setColor(highlight);
        g2d.drawString(getText(), textX, textY - gap);
